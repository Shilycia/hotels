...
<head>
    ...
    <style>
        /* ... (Semua CSS CSS variabel dan style milikmu tetap sama persis) ... */
        :root {
            --sand: #f7f3ee; --sand2: #ede8e0; --sand3: #e4ddd3; --stone: #c8bfb0;
            --bark: #8b7355; --bark-soft: #f4ede4; --moss: #4a7c59; --moss-soft: #edf4ef;
            --clay: #c07850; --clay-soft: #fdf0e8; --slate: #5a6a7a; --slate-soft: #eef1f5;
            --ink: #2c2420; --ink2: #6b5e54; --ink3: #9e9088; --sidebar-bg: #2c2820;
            --sidebar-w: 220px; --surface: #ffffff; --radius: 10px; --radius-sm: 7px;
            --shadow-sm: 0 1px 3px rgba(44,36,32,0.06); --shadow: 0 4px 16px rgba(44,36,32,0.08);
            --transition: .2s ease;
        }

        *, ::before, ::after { box-sizing: border-box; margin: 0; padding: 0; }
        html { scroll-behavior: smooth; }
        body { font-family: 'DM Sans', sans-serif; background: var(--sand); color: var(--ink); min-height: 100vh; display: flex; overflow-x: hidden; font-size: 13.5px; }
        
        .sidebar { width: var(--sidebar-w); min-height: 100vh; background: var(--sidebar-bg); position: fixed; top: 0; left: 0; z-index: 100; display: flex; flex-direction: column; transition: transform var(--transition); overflow-y: auto; }
        .sidebar-brand { padding: 22px 18px 18px; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid rgba(255,255,255,0.07); }
        .brand-icon { width: 34px; height: 34px; border-radius: 8px; background: #c8a96e; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .sidebar-section { padding: 16px 18px 5px; font-size: 9.5px; font-weight: 500; color: rgba(240,235,227,0.25); letter-spacing: 1.2px; text-transform: uppercase; }
        .nav-item { display: flex; align-items: center; gap: 9px; padding: 8px 18px; margin: 1px 6px; border-radius: var(--radius-sm); color: rgba(240,235,227,0.45); font-size: 12.5px; font-weight: 400; text-decoration: none; cursor: pointer; transition: all var(--transition); position: relative; }
        ...
        .nav-item.active::before { content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 2.5px; height: 55%; background: #c8a96e; border-radius: 0 2px 2px 0; }
        ...
        .user-avatar { width: 30px; height: 30px; border-radius: 50%; background: rgba(200,169,110,0.25); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 500; color: #e2c98a; flex-shrink: 0; }
        ...
        .logout-btn { margin-left: auto; background: none; border: none; color: rgba(240,235,227,0.2); cursor: pointer; font-size: 12px; padding: 4px; border-radius: 4px; transition: color var(--transition); }
        .logout-btn:hover { color: var(--clay); }

        .main { margin-left: var(--sidebar-w); flex: 1; min-height: 100vh; display: flex; flex-direction: column; }
        .topbar { background: var(--sand); border-bottom: 1px solid var(--sand2); padding: 0 26px; height: 54px; display: flex; align-items: center; gap: 10px; position: sticky; top: 0; z-index: 50; }
        ...
        .topbar-btn { width: 32px; height: 32px; border-radius: var(--radius-sm); border: 1px solid var(--sand2); background: transparent; display: flex; align-items: center; justify-content: center; color: var(--ink3); cursor: pointer; font-size: 13px; transition: all var(--transition); position: relative; }
        ...
        /* Komponen yang sudah kamu definisikan di-copy persis di sini... */
        .stats-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px,1fr)); gap: 12px; margin-bottom: 18px; }
        .stat-card { background: var(--surface); border: 1px solid var(--sand2); border-radius: var(--radius); padding: 16px 18px; transition: transform var(--transition), box-shadow var(--transition); }
        ...
        .stat-icon { width: 30px; height: 30px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 12px; flex-shrink: 0; }
        ...
        .chart-card { background: var(--surface); border: 1px solid var(--sand2); border-radius: var(--radius); padding: 18px 20px; }
        ...
        .table-card { background: var(--surface); border: 1px solid var(--sand2); border-radius: var(--radius); overflow: hidden; margin-bottom: 18px; }
        .table-card-header { padding: 14px 20px; border-bottom: 1px solid var(--sand2); display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px; }
        ...
        .search-wrap { display: flex; align-items: center; gap: 7px; background: var(--sand); border: 1px solid var(--sand2); border-radius: var(--radius-sm); padding: 0 10px; }
        .search-wrap i { font-size: 11px; color: var(--ink3); }
        .search-input { border: none; background: transparent; font-family: 'DM Sans', sans-serif; font-size: 12px; color: var(--ink); outline: none; padding: 7px 0; width: 140px; }
        table { width: 100%; border-collapse: collapse; }
        thead { background: var(--sand); }
        th { padding: 9px 20px; font-size: 10px; font-weight: 500; color: var(--ink3); text-align: left; letter-spacing: 0.5px; text-transform: uppercase; white-space: nowrap; }
        td { padding: 11px 20px; font-size: 12.5px; color: var(--ink2); border-top: 1px solid var(--sand2); }
        ...
        .btn { padding: 7px 14px; border: none; border-radius: var(--radius-sm); font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 500; cursor: pointer; display: inline-flex; align-items: center; gap: 5px; transition: all var(--transition); }
        ...
        .btn-sm { padding: 5px 10px; font-size: 11px; text-decoration: none; }
        .alert { display: flex; align-items: center; gap: 8px; padding: 10px 16px; border-radius: var(--radius-sm); font-size: 12.5px; }
...
<body>
    <aside class="sidebar">
        ...
        <a href="{{ route('admin.reports.index') }}" class="nav-item {{ request()->routeIs('admin.reports.*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fas fa-chart-line"></i></span> Laporan Keuangan
        </a>
        ...
        <a href="{{ route('admin.bookings.index') }}" class="nav-item {{ request()->routeIs('admin.bookings.*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fas fa-calendar-check"></i></span> Reservasi
        </a>
        <a href="{{ route('admin.rooms.index') }}" class="nav-item {{ request()->routeIs('admin.rooms.*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fas fa-door-closed"></i></span> Kamar Fisik
        </a>
        ...
        <div class="sidebar-section">Restoran & Paket</div>
        <a href="{{ route('admin.orders.index') }}" class="nav-item {{ request()->routeIs('admin.orders.*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fas fa-receipt"></i></span> Pesanan F&B
        </a>
        <a href="{{ route('admin.menus.index') }}" class="nav-item {{ request()->routeIs('admin.menus.*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fas fa-utensils"></i></span> Menu Restoran
        </a>
        <a href="{{ route('admin.packages.index') }}" class="nav-item {{ request()->routeIs('admin.packages.*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fas fa-box-open"></i></span> Paket Bundling
        </a>

        <div class="sidebar-section">Keuangan & Sistem</div>
        <a href="{{ route('admin.payments.index') }}" class="nav-item {{ request()->routeIs('admin.payments.*') ? 'active' : '' }}">
            <span class="nav-icon"><i class="fas fa-credit-card"></i></span> Pembayaran
        </a>
